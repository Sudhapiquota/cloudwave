...
function parseLunrResults(results) {
    ...
    for (var i = 0; i < results.length; i++) {
        ...
        var preview = item["p"];
        var link = item["l"];
        var result = ('<p class="h3"><span class="result-title "><a class=" " href="' + link + '">'
                    + title + '</a><br></span><br><span class="result-preview">'
                    + preview + '</span></p>');
        html.push(result);
    }
    if (html.length) {
        ...
        return "<p class='h3'>Your search returned no results.</p>";
    }
}
...
function showResultCount(query, total, domElementId) {
    ...
    var found = "<p class='h3'>Found " + total + " result" + s;
    if (query != "" && query != null) {
        query = escapeHtml(query);
        var forQuery = ' for <span class="result-query h3 text-uppercase">' + decodeURIComponent(query) + '</span>';
    }
    else {
        var forQuery = "";
    }
    var element = document.getElementById(domElementId);
    element.innerHTML = found + forQuery + "</p><br>";
}
...
window.onload = function() {
    ...
    if (query != "" && query != null) {
        ...
        searchLunr(encodeURIComponent(query));
    }
};